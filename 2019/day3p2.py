...
class Grid():
    def __init__(self):
        self.grid = []
        self.intersections = []
        self.position = {'w1': {'x': 0, 'y': 0}, 'w2': {'x': 0, 'y': 0}}
        self.steps = {'w1': 0, 'w2': 0}
    def append(self, pt, wire):
        self.steps[wire] += 1
        for i in range(len(self.grid)):
            if self.grid[i][0] == pt:
                if self.grid[i][1] == wire:
                    return 0
                else:
                    if pt not in self.intersections:
                        self.intersections.append([pt, self.grid[i][2] + self.steps[wire]])
                        return 1
                    return 0
        self.grid.append([pt, wire, self.steps[wire]])
        return 0

def parse(i, wirename):
    d = i[0]
    l = int(i[1:])
    if d == 'U':
        x0, x1, y0, y1, step = grid.position[wirename]['x'], grid.position[wirename]['x'] + 1, grid.position[wirename]['y'] + 1, grid.position[wirename]['y'] + l + 1, 1
        grid.position[wirename]['y'] = grid.position[wirename]['y'] + l
    elif d == 'D':
        x0, x1, y0, y1, step = grid.position[wirename]['x'], grid.position[wirename]['x'] - 1, grid.position[wirename]['y'] - 1, grid.position[wirename]['y'] - l - 1, -1
        grid.position[wirename]['y'] = grid.position[wirename]['y'] - l
    elif d == 'R':
        x0, x1, y0, y1, step = grid.position[wirename]['x'] + 1, grid.position[wirename]['x'] + l + 1, grid.position[wirename]['y'], grid.position[wirename]['y'] + 1, 1
        grid.position[wirename]['x'] = grid.position[wirename]['x'] + l
    elif d == 'L':
        x0, x1, y0, y1, step = grid.position[wirename]['x'] - 1, grid.position[wirename]['x'] - l - 1, grid.position[wirename]['y'], grid.position[wirename]['y'] - 1, -1
        grid.position[wirename]['x'] = grid.position[wirename]['x'] - l
    for x in range(x0, x1, step):
        for y in range(y0, y1, step):
            if grid.append((x, y), wirename):
                print("Intersection: {}\nCombined steps: {}".format((x, y), grid.intersections[-1][-1]))

grid = Grid()

def testWire(wirename1, wirename2):
    for i in range(301):
        print(i, '/ 301')
        i1, i2 = wire1[i], wire2[i]
        parse(i1, 'w1')
        parse(i2, 'w2')
